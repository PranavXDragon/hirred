...
export async function saveCompanyProfile(formData) {
  ...
  companyData.slug = `${baseSlug}-${Math.random().toString(36).substring(2, 6)}`;
...
export async function postJob(formData) {
  ...
  jobData.slug = `${baseSlug}-${Math.random().toString(36).substring(2, 6)}`;
...
export async function getEmployerJobApplicants(jobId) {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return [];
